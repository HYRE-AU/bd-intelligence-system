/**
 * Test the research agent + GPT-4o synthesis in isolation.
 * No HN polling, no Supabase, no emails.
 *
 * Usage:
 *   npx ts-node scripts/test-research-agent.ts                  # uses Airbnb
 *   npx ts-node scripts/test-research-agent.ts <company_name>   # custom company
 */
...
(async () => {
  ...
  console.log(`\n=== Research Agent Test — "${companyName}" ===\n`);
  ...
  // Run research agent (dummy story ID — we're not testing HN thread extraction)
